...
def spectrogram(RXLO = int(433.5e6), RXFS=int(30e6), samples = 500, fft_size = 32768, affichage = False):
    ...
    rx_samples1 = np.concatenate(np.copy(rx_samples)).ravel()
    num_rows = samples
    ...
    if affichage :
        ...
        plt.plot(np.linspace(0,len(rx_samples1),len(rx_samples1)),rx_samples1)
        plt.show()

    return rx_samples


def max_freq(RXLO = int(430e6), RXFS=int(30e6), samples = 500, fft_size = 32768, affichage = False) :

    print("Début de l'acquisition du spectrogramme")
    ...
    m_freq = RXLO - RXFS / 2 + max_freq_index * RXFS / fft_size
    return m_freq
...
print(max_freq(RXLO = int(2400e6), RXFS=int(30e6), samples = 500, fft_size = 32768, affichage = True))
